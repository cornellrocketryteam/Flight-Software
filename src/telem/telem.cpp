...
void Telem::pack_data() {
    ...
    metadata_bitfield |= (static_cast<uint8_t>(state->actuator.sv_open) & 0b1) << 12;
    metadata_bitfield |= (static_cast<uint8_t>(state->actuator.mav_open) & 0b1) << 11;

    metadata_bitfield |= (static_cast<uint8_t>(state->gps.fresh) & 0b1) << 10;
